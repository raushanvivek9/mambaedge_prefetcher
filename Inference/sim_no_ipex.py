import os, hashlib, time, json, lzma, math
import numpy as np
import torch
...
def champsim_simulate(
    ...
    print(); print('='*70)
    print(f'  MambaEdge ChampSim Simulation  |  Trace: {name}  |  Degree: {prefetch_degree}x')
    print('='*70)
    t_total = time.time()
    ...
    for i in range(seq_len, N):
        ...
        s_pf_issued += prefetch_degree
        s_correct   += int(hit)
        s_page_correct += int(page_hit)
        if hit:
    ...
    page_coverage  = s_page_correct / max(1, s_accesses)           * 100
    ...
    print(f'  Page Coverage              : {page_coverage:>10.2f}%')
...
for tp in TRACE_PATHS:
    r = champsim_simulate(tp, seq_len=SEQ_LEN, warmup_instr=200_000, sim_instr=500_000,
                          batch_size=512, prefetch_degree=PREFETCH_DEGREE)
...
if all_results:
    ...
    print('='*163)
    print(f"  {'Trace':<30} {'AddrAcc':>8} {'PageCov':>8} {'Useful':>10} {'Acc%':>7} "
          f"{'Useless':>10} {'Over%':>7} {'MissRed':>8} {'AMAT%':>7} "
          f"{'BW%':>10} {'EnrgSav%':>9} {'PwrSav%':>8} {'Pollution%':>10} {'IPCSpd':>10}")
    print('  ' + '-'*161)
    for r in all_results:
        cm = r['cache_metrics']
        print(f"  {r['trace'][:30]:<30} "
              f"{r['addr_accuracy']:>7.2f}% "
              f"{r['page_coverage']:>7.2f}% "
              f"{r['pf_useful']:>10,} "
              f"{r['pf_accuracy']:>6.2f}% "
              f"{r['pf_useless']:>10,} "
              f"{r['overprediction']:>6.2f}% "
              f"{cm['miss_reduction_pct']:>7.2f}% "
              f"{cm['amat_reduction_pct']:>6.2f}% "
              f"{cm['throughput_gain_pct']:>9.2f}% "
              f"{cm['energy_savings_pct']:>8.2f}% "
              f"{cm['power_savings_pct']:>7.2f}% "
              f"{cm['pollution_rate_pct']:>9.2f}% "
              f"{r['ipc_speedup']:>9.3f}x")
    ...
    print('  ' + '-'*161)
    print(f"  {'AVERAGE':<30} "
          f"{gavg('addr_accuracy'):>7.2f}% "
          f"{gavg('page_coverage'):>7.2f}% "
          f"{gavg('pf_useful'):>10,.0f} "
          f"{gavg('pf_accuracy'):>6.2f}% "
          f"{gavg('pf_useless'):>10,.0f} "
          f"{gavg('overprediction'):>6.2f}% "
          f"{gavg('miss_reduction_pct', True):>7.2f}% "
          f"{gavg('amat_reduction_pct', True):>6.2f}% "
          f"{gavg('throughput_gain_pct', True):>9.2f}% "
          f"{gavg('energy_savings_pct', True):>8.2f}% "
          f"{gavg('power_savings_pct', True):>7.2f}% "
          f"{gavg('pollution_rate_pct', True):>9.2f}% "
          f"{gavg('ipc_speedup'):>9.3f}x")
    print('='*163)

    out_json = '/home/cs25m115/mamba_inf/mambaedge_no_ipex_full_metrics_results_champ.json'
